...
function Calculator({ miners }: { miners: MonitorMiner[] }) {
  ...
  return <section className="lm-calc">
    <div className="lm-panel"><div className="lm-panel-head"><h2>Parâmetros da estimativa</h2><div className="lm-c-mode"><button className={mode === "network" ? "active" : ""} onClick={() => setMode("network")}>Rede (BTC)</button><button className={mode === "flat" ? "active" : ""} onClick={() => setMode("flat")}>US$/TH (Rental Hash)</button></div></div>
      <div className="lm-calc-params">{mode === "network" ? <><label>Rendimento da rede<input type="number" step=".00001" value={rate} onChange={(e) => setRate(+e.target.value)} /><small>BTC / PH/s · dia</small></label><label>Cotação do BTC<input type="number" value={btc} onChange={(e) => setBtc(+e.target.value)} /><small>{quoting ? "buscando…" : quote?.ok ? `${quote.source === "coingecko" ? "CoinGecko" : "API-Ninjas"} · agora` : quote?.note ?? "valor em reais"} · <button type="button" className="lm-quote-refresh" onClick={fetchQuote} disabled={quoting}>atualizar</button></small></label></> : <><label>Quanto vale sua potência<input type="number" step=".001" value={usd} onChange={(e) => setUsd(+e.target.value)} /><small>US$ / TH/s por dia</small></label><label>Bônus no pagamento<select value={bonus} onChange={(e) => setBonus(+e.target.value)}><option value="0">0%</option><option value="5">5%</option><option value="7.5">7,5%</option><option value="10">10%</option></select></label><label>Câmbio US$→R$<span className="lm-live-value">{fxQuoting ? "buscando…" : usdBrl != null ? `R$ ${usdBrl.toFixed(4)}` : "indisponível"}</span><small>CoinGecko · <button type="button" className="lm-quote-refresh" onClick={fetchFx} disabled={fxQuoting}>atualizar</button></small></label></>}<label>Hashrate<span className="lm-live-value">{th.toFixed(2)} TH/s</span><small>frota online ao vivo</small></label><label>Devfee do firmware<input type="number" step=".1" value={fee} onChange={(e) => setFee(+e.target.value)} /><small>exceto Avalon</small></label></div>
    </div>
    <div className="lm-results">{mode === "network" ? <><Result css="lm-r-btc" label="Rendimento em BTC" value={`${dailyBtc.toFixed(8)} BTC/dia`} sub={`${(dailyBtc * 30).toFixed(8)} BTC/mês`} /><Result css="lm-r-net" label="Líquido/dia" value={`${money(dailyBrl, "BRL")}/dia`} sub={`${money(dailyBrl * 30, "BRL")}/mês`} /></> : <><Result css="lm-r-rev" label="Pagamento / hora" value={`${money(dailyUsd / 24, "USD")}/h${brlEquivalent(dailyUsd / 24)}`} /><Result css="lm-r-net" label="Pagamento / dia" value={`${money(dailyUsd, "USD")}/dia${brlEquivalent(dailyUsd)}`} sub={`${money(dailyUsd * 30, "USD")}/mês${brlEquivalent(dailyUsd * 30)}`} /></>}</div>
    <div className="lm-panel lm-income"><div className="lm-panel-head"><h2>Rendimento por máquina</h2></div><div className="lm-scroll"><table><thead><tr><th>Máquina</th><th>Tipo</th><th>Devfee</th><th>TH/s</th><th>{mode === "network" ? "BTC/dia" : "US$/dia"}</th>{mode === "flat" && <th>R$/dia</th>}</tr></thead><tbody>{online.map((miner) => { const usdPerDay = usd * effective(miner) * (1 + bonus / 100); return <tr key={miner.id}><td>{miner.name}</td><td><span className="lm-tag">{miner.type}</span></td><td className="lm-dim">{miner.type === "avalon" ? "isento" : `${fee.toFixed(1)}%`}</td><td className="lm-num">{hash(miner.hashrate_ths)}</td><td className="lm-num lm-pos">{mode === "network" ? (rate * effective(miner) / 1000).toFixed(8) : money(usdPerDay, "USD")}</td>{mode === "flat" && <td className="lm-num lm-dim">{usdBrl != null ? money(usdPerDay * usdBrl, "BRL") : "—"}</td>}</tr>; })}</tbody></table></div></div><p className="lm-note">Estimativa. O rendimento real varia com dificuldade da rede, sorte do pool, taxas e uptime. Mês = 30 dias.</p>
  </section>;
}
...
function MinerRow({ miner, open }: { miner: MonitorMiner; open: () => void }) {
  if (miner.licensed === false) return <tr className="lm-unlicensed" onClick={open}><td><span className="lm-dot lm-off" /><b>{miner.name}</b></td><td className="lm-num lm-dim">{miner.ip}</td><td><span className="lm-tag">{miner.type}</span></td><td colSpan={7} className="lm-license-cell">🔒 Aguardando licença — <Link href="/billing" onClick={(e) => e.stopPropagation()}>comprar</Link></td></tr>;
...
function MinerCard({ miner, open }: { miner: MonitorMiner; open: () => void }) {
  if (miner.licensed === false) return <article className="lm-mcard lm-unlicensed" onClick={open}><div className="lm-mc-top"><span className="lm-dot lm-off" /><div><b>{miner.name}</b><small>{miner.ip} · {miner.type}</small></div></div><p className="lm-license-cell">🔒 Aguardando licença — <Link href="/billing" onClick={(e) => e.stopPropagation()}>comprar</Link></p></article>;
...
function Modal({ miner, close, addAction, changePool, deleteMiner, rebootMiner }: { miner: MonitorMiner | null; close: () => void; addAction: (formData: FormData) => void | Promise<void>; changePool: (minerId: string) => void; deleteMiner: (minerId: string) => void; rebootMiner: (minerId: string) => Promise<RebootResult> }) {
  ...
  return <div className="lm-overlay" onMouseDown={(e) => e.target === e.currentTarget && close()}><div className="lm-modal"><button className="lm-close" onClick={close}>×</button>
    {miner ? <>
      <h3>{miner.name}</h3>
      <p className="lm-sub">{miner.type} · {miner.ip}:{miner.port} · {miner.licensed === false ? "aguardando licença" : miner.online ? "online" : "OFFLINE"}{miner.error && ` · ${miner.error}`}</p>
      {miner.licensed === false
        ? <p className="lm-license-cell">🔒 Essa máquina está cadastrada, mas fora da sua licença atual — não coleta nem mostra dados até você <Link href="/billing">comprar mais licenças</Link>.</p>
        : <>
            <div className="lm-kv"><Detail label="Modelo" value={miner.model ?? "—"} /><Detail label="Hashrate atual" value={`${hash(miner.hashrate_ths)} TH/s`} /><Detail label="Hashrate médio" value={`${hash(miner.hashrate_avg_ths)} TH/s`} /><Detail label="Consumo" value={`${watts(miner.power_w)} W`} /><Detail label="Tensão / Corrente" value={<VA miner={miner} />} /><Detail label="Eficiência" value={`${n(miner.efficiency_jth)?.toFixed(1) ?? "—"} J/TH`} /><Detail label="Refrigeração" value={<Cooling miner={miner} full />} /><Detail label="Uptime" value={uptime(miner.uptime_s)} /><Detail label="Shares aceitas" value={`${n(miner.accepted) ?? 0}`} /><Detail label="Shares rejeitadas" value={`${n(miner.rejected) ?? 0}`} /><Detail label="Pool" value={miner.pool ?? "—"} /><Detail label="Worker" value={miner.worker ?? "—"} /></div>
            <h4>Temperatura por hashboard</h4><ThermalStrip miner={miner} large />
            {rebootFeedback && <p className={`lm-command-feedback ${rebootFeedback.ok ? "ok" : "error"}`}>{rebootFeedback.message}</p>}
            <div className="lm-detail-actions"><button className="lm-btn lm-pool-button" onClick={() => changePool(miner.id)}>⇄ Trocar pool desta máquina</button><button className="lm-btn" disabled={rebooting} onClick={doReboot}>{rebooting ? "Reiniciando…" : "⟲ Reiniciar máquina"}</button></div>
          </>}
      <div className="lm-detail-actions"><button className="lm-btn lm-danger" onClick={doDelete}>🗑 Excluir máquina</button></div>
    </> : <><h3>Adicionar máquina</h3><p className="lm-sub">a próxima varredura já inclui o dispositivo</p><form action={addAction} onSubmit={close}><label>Nome<input name="name" required placeholder="ex: ASIC-01" /></label><label>IP<input name="ip" required placeholder="192.168.1.101" /></label><label>Tipo<select name="type" defaultValue="antminer"><option value="antminer">Antminer</option><option value="whatsminer">Whatsminer</option><option value="avalon">Avalon</option></select></label><label>Porta<input name="port" type="number" defaultValue="4028" /></label><button className="lm-btn lm-primary" type="submit">Adicionar</button></form></>}
  </div></div>;
}
