...
import type { RemoteResult, RemoteToggleResult } from "./remote-access";
...
type SortKey = "name" | "ip" | "type" | "hashrate_ths" | "power_w" | "uptime_s" | "accepted" | "rejected";
...
type OpenRemote = (minerId: string) => Promise<RemoteResult>;

/** Abre a tela da ASIC em outra aba pelo acesso remoto. Devolve a mensagem de erro, ou null se abriu. */
async function openRemoteTab(openRemote: OpenRemote, minerId: string): Promise<string | null> {
  // A aba precisa ser aberta no clique (síncrono), senão o navegador bloqueia o pop-up depois do await.
  const tab = window.open("about:blank", "_blank");
  const result = await openRemote(minerId);
  if (result.ok) {
    if (tab) { tab.opener = null; tab.location.href = result.url; } else window.location.assign(result.url);
    return null;
  }
  tab?.close();
...
/** Botão 🌐 ao lado do IP: abre a máquina de fora da rede local sem passar pelo modal. */
function RemoteButton({ minerId, openRemote }: { minerId: string; openRemote: OpenRemote }) {
  const t = useTranslations("farmDetail");
  const [busy, setBusy] = useState(false);
  const go = async (event: React.MouseEvent) => {
    event.stopPropagation();
    setBusy(true);
    const error = await openRemoteTab(openRemote, minerId);
    setBusy(false);
    if (error) window.alert(error);
  };
  return <button type="button" className="lm-remote-btn" disabled={busy} title={t("remoteOpenMachineHint")} aria-label={t("remoteOpenMachine")} onClick={go}>{busy ? "…" : <>🌐<span>{t("remoteShort")}</span></>}</button>;
}
...
function Calculator({ miners }: { miners: MonitorMiner[] }) {
  const t = useTranslations("farmDetail");
  const locale = useLocale();
  const [mode, setMode] = useState<"network" | "flat">("network"), [rate, setRate] = useState(.00056), [btc, setBtc] = useState(350000), [usd, setUsd] = useState(.04), [bonus, setBonus] = useState(0);
  ...
  // Sem devfee individual definido pra essa maquina, nao descontar nada - o
  // padrao seguro e assumir firmware original (sem devfee), nao um percentual
  // generico por fabricante (isso era impreciso: devfee depende do firmware
  // instalado, nao do fabricante do hardware).
  const effective = (miner: MonitorMiner) => (n(miner.hashrate_ths) ?? 0) * (1 - (miner.devfee_pct ?? 0) / 100);
  ...
  return <section className="lm-calc">
    ...
    <div className="lm-panel lm-income"><div className="lm-panel-head"><h2>{t("incomePerMachine")}</h2><p className="lm-note">{t("devfeePerMachineHint")}</p></div><div className="lm-scroll table-wrap"><table><thead><tr><th>{t("colMachine2")}</th><th>{t("colType2")}</th><th>{t("colDevfee")}</th><th>TH/s</th><th>{mode === "network" ? "BTC/dia" : "US$/dia"}</th>{mode === "flat" && <th>R$/dia</th>}</tr></thead><tbody>{online.map((miner) => { const usdPerDay = usd * effective(miner) * (1 + bonus / 100); return <tr key={miner.id}><td>{miner.name}</td><td><span className="lm-tag">{miner.type}</span></td><td className="lm-dim">{miner.devfee_pct != null ? `${miner.devfee_pct.toLocaleString(locale, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}%` : t("exempt")}</td><td className="lm-num" data-label="TH/s">{hash(miner.hashrate_ths)}</td><td className="lm-num lm-pos">{mode === "network" ? (rate * effective(miner) / 1000).toFixed(8) : money(usdPerDay, "USD")}</td>{mode === "flat" && <td className="lm-num lm-dim">{usdBrl != null ? money(usdPerDay * usdBrl, "BRL") : "—"}</td>}</tr>; })}</tbody></table></div></div><p className="lm-note">{t("estimateNote")}</p>
  </section>;
}
...
export function LegacyMonitor({ farmId, farmName, timezone, miners, history, events, initialMinerId, poolCommands, addMinerAction, deleteMinerAction, rebootMinerAction, stopMiningAction, updateDevfeeAction, agentPanel, remoteAccess, openRemoteAction, setRemoteAccessAction }: { farmId: string; farmName: string; timezone: string | null; miners: MonitorMiner[]; history: HistoryPoint[]; events: EventRow[]; initialMinerId: string | null; poolCommands: PoolCommandSummary[]; addMinerAction: (formData: FormData) => void | Promise<void>; deleteMinerAction: (minerId: string) => void | Promise<void>; rebootMinerAction: (minerId: string) => Promise<RebootResult>; stopMiningAction: (minerId: string) => Promise<RebootResult>; updateDevfeeAction: (minerId: string, devfeePct: number | null) => Promise<DevfeeResult>; agentPanel: ReactNode; remoteAccess: { enabled: boolean; canUse: boolean }; openRemoteAction: (minerId: string) => Promise<RemoteResult>; setRemoteAccessAction: (enabled: boolean) => Promise<RemoteToggleResult> }) {
  const t = useTranslations("farmDetail");
  ...
  return <div className="legacy-monitor">
    <header className="lm-header"><Link className="lm-brand" href="/dashboard"><img src="/favicon.svg" width="38" height="38" alt="" /><span><b>ASIC</b><em>Monitor</em></span></Link><nav className="lm-viewtoggle"><button className={view === "table" ? "active" : ""} onClick={() => setView("table")}>{t("fleet")}</button><button className={view === "cards" ? "active" : ""} onClick={() => setView("cards")}>{t("cards")}</button><button className={view === "calc" ? "active" : ""} onClick={() => setView("calc")}>{t("income")}</button></nav><div className="lm-status"><span>●</span> {t("onlineStatus", { online: online.length, total: miners.length, age: age(latest) })}{pending && t("updating")}</div><button className="lm-logout lm-hl" onClick={() => setSetup(!setup)}>{t("setupButton")}</button><button className="lm-logout lm-hl lm-hl-solid" onClick={goBack}>← {t("farmsLink")}</button></header>
    <main className="lm-wrap"><div className="lm-farm"><b>{farmName}</b>{timezone && <span>{timezone}</span>}</div><section className="lm-kpis"><Kpi css="lm-k-hash" label={t("totalHashrate")} value={totalHash.toFixed(2)} unit="TH/s" /><Kpi css="lm-k-pow" label={t("totalConsumption")} value={(totalPower / 1000).toFixed(2)} unit="kW" /><Kpi css="lm-k-eff" label={t("efficiency")} value={totalHash ? (totalPower / totalHash).toFixed(1) : "—"} unit="J/TH" /><Kpi css="lm-k-on" label={t("online")} value={`${online.length}`} unit={`/ ${miners.length}`} /><Kpi css="lm-k-off" label={t("offline")} value={`${miners.length - online.length}`} /></section>
      ...
      {view === "calc" ? <Calculator miners={miners} /> : <><div className="lm-bar"><input type="search" placeholder={t("filterPlaceholder")} value={query} onChange={(e) => setQuery(e.target.value)} /><select value={type} onChange={(e) => setType(e.target.value)}><option value="">{t("allTypes")}</option><option value="antminer">Antminer</option><option value="whatsminer">Whatsminer</option><option value="avalon">Avalon</option></select><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="">{t("onlineAndOffline")}</option><option value="on">{t("onlyOnline")}</option><option value="off">{t("onlyOffline")}</option></select><div className="lm-sortbox"><select className="lm-sortselect" aria-label={t("sortBy")} value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)}><option value="name">{t("colMachine")}</option><option value="ip">{t("colIp")}</option><option value="type">{t("colType")}</option><option value="hashrate_ths">TH/s</option><option value="power_w">{t("consumptionLabel")}</option><option value="uptime_s">{t("colUptime")}</option><option value="accepted">{t("colShares")}</option><option value="rejected">{t("colRejected")}</option></select><button type="button" className="lm-btn lm-sortdir" aria-label={t("sortDirection")} onClick={() => setDirection(-direction)}>{direction > 0 ? "▲" : "▼"}</button></div><span /><button className="lm-btn lm-pool-button" onClick={() => { setPoolMinerId(null); setPoolOpen(true); }}>⇄ {t("changePool")}</button><button className="lm-btn lm-primary" onClick={() => setAddOpen(true)}>+ {t("addMachine")}</button><button className="lm-btn" onClick={() => startTransition(() => router.refresh())}>↻ {t("refresh")}</button></div>{view === "table" ? <div className="lm-tablewrap"><div className="lm-scroll"><table><thead><tr>{th(t("colMachine"), "name")}{th(t("colIp"), "ip")}{th(t("colType"), "type")}{th("TH/s", "hashrate_ths")}{th(t("consumptionLabel"), "power_w")}{th(t("colHashboards"))}{th(t("colCooling"))}{th(t("colUptime"), "uptime_s")}{th(t("colShares"), "accepted")}{th(t("colRejected"), "rejected")}</tr></thead><tbody>{rows.length ? rows.map((miner) => <MinerRow key={miner.id} miner={miner} open={() => setSelected(miner)} remote={remoteAccess.enabled && remoteAccess.canUse ? openRemoteAction : null} />) : <tr><td colSpan={10} className="lm-empty">{t("noMachinesMatch")}</td></tr>}</tbody></table></div></div> : <div className="lm-cards">{rows.map((miner) => <MinerCard key={miner.id} miner={miner} open={() => setSelected(miner)} remote={remoteAccess.enabled && remoteAccess.canUse ? openRemoteAction : null} />)}</div>}</>}
      {setup && <section className="lm-setup"><RemoteAccessPanel enabled={remoteAccess.enabled} canManage={remoteAccess.canUse} setEnabled={setRemoteAccessAction} />{agentPanel}</section>}
      <PoolCommandHistory commands={poolCommands} />
    </main>{(selected || addOpen) && <Modal miner={selected} events={selected ? events.filter((e) => e.miner_id === selected.id) : []} close={() => { setSelected(null); setAddOpen(false); }} addAction={addMinerAction} changePool={(minerId) => { setSelected(null); setPoolMinerId(minerId); setPoolOpen(true); }} deleteMiner={removeMiner} rebootMiner={restartMiner} stopMining={stopMining} updateDevfee={updateDevfeeAction} remoteReady={remoteAccess.enabled && remoteAccess.canUse} openRemote={openRemoteAction} />}
...
function MinerRow({ miner, open, remote }: { miner: MonitorMiner; open: () => void; remote: OpenRemote | null }) {
  const t = useTranslations("farmDetail");
  const locale = useLocale();
  ...
  return <tr onClick={open}><td><span className={`lm-dot ${healthDotClass(miner)}`} /><b>{miner.name}</b></td><td className="lm-num lm-dim"><a className="lm-iplink" href={`http://${miner.ip}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}>{miner.ip}</a>{remote && <RemoteButton minerId={miner.id} openRemote={remote} />}</td><td><span className="lm-tag">{miner.type}</span></td><td className="lm-num" data-label="TH/s">{hash(miner.hashrate_ths)}</td><td className="lm-num" data-label={t("consumptionLabel")}>{watts(miner.power_w, locale)} {miner.power_estimated && <span className="lm-est">{t("powerEstimatedTag")}</span>}</td><td><ThermalStrip miner={miner} /></td><td><Cooling miner={miner} /></td><td className="lm-num lm-dim" data-label={t("colUptime")}>{uptime(miner.uptime_s)}</td><td className="lm-num" data-label={t("colShares")}>{n(miner.accepted)?.toLocaleString(locale) ?? "—"}</td><td className={`lm-num ${n(miner.rejected) ? "lm-warm" : "lm-dim"}`} data-label={t("colRejected")}>{n(miner.rejected) ?? 0}</td></tr>;
}
function MinerCard({ miner, open, remote }: { miner: MonitorMiner; open: () => void; remote: OpenRemote | null }) {
  const t = useTranslations("farmDetail");
  const locale = useLocale();
  ...
  return <article className={`lm-mcard ${miner.online ? "" : "lm-offline"}`} onClick={open}><div className="lm-mc-top"><span className={`lm-dot ${healthDotClass(miner)}`} /><div><b>{miner.name}</b><small>{miner.ip} · {miner.model ?? miner.type}</small></div>{remote && <RemoteButton minerId={miner.id} openRemote={remote} />}<span className="lm-tag">{miner.type}</span></div><div className="lm-mc-metrics"><div><span>{t("hashrateLabel")}</span><strong>{hash(miner.hashrate_ths)}<small> TH/s</small></strong></div><div><span>{t("consumptionLabel")}</span><strong>{watts(miner.power_w, locale)}<small> W</small></strong></div></div><p className="lm-strip-label">{t("hashboardsLabel")}</p><ThermalStrip miner={miner} large /><div className="lm-card-cooling"><Cooling miner={miner} /></div><footer><span>{t("colShares")} {n(miner.accepted)?.toLocaleString(locale) ?? "—"}</span><span>{uptime(miner.uptime_s)}</span><span>{t("rejectedShort", { count: n(miner.rejected) ?? 0 })}</span></footer></article>;
}
function DevfeeEditor({ miner, updateDevfee }: { miner: MonitorMiner; updateDevfee: (minerId: string, devfeePct: number | null) => Promise<DevfeeResult> }) {
  const t = useTranslations("farmDetail");
  const [value, setValue] = useState(miner.devfee_pct != null ? String(miner.devfee_pct) : "");
  const [saving, setSaving] = useState(false);
  const [feedback, setFeedback] = useState<DevfeeResult | null>(null);
  const save = async () => {
    // Aceita virgula como separador decimal (ex.: "2,8") alem de ponto - o
    // <input type="number"> nativo rejeita virgula no Chrome mesmo com o
    // navegador em pt-BR, o que fazia o valor parecer invalido/nao salvar.
    const normalized = value.trim().replace(",", ".");
    if (normalized !== "" && !Number.isFinite(Number(normalized))) { setFeedback({ ok: false, message: t("devfeeInvalid") }); return; }
    setSaving(true); setFeedback(null);
    const result = await updateDevfee(miner.id, normalized === "" ? null : Number(normalized));
    setSaving(false);
    setFeedback(result);
  };
  return <div className="lm-devfee">
    <label>{t("devfeeLabel")}<input type="text" inputMode="decimal" placeholder={t("devfeeNonePlaceholder")} value={value} onChange={(e) => setValue(e.target.value)} /></label>
    <button className="lm-btn" type="button" disabled={saving} onClick={save}>{saving ? t("devfeeSaving") : t("devfeeSave")}</button>
    <p className="lm-devfee-hint">{t("devfeeHint")}</p>
...
function Modal({ miner, events, close, addAction, changePool, deleteMiner, rebootMiner, stopMining, updateDevfee, remoteReady, openRemote }: { remoteReady: boolean; openRemote: (minerId: string) => Promise<RemoteResult>; miner: MonitorMiner | null; events: EventRow[]; close: () => void; addAction: (formData: FormData) => void | Promise<void>; changePool: (minerId: string) => void; deleteMiner: (minerId: string) => void; rebootMiner: (minerId: string) => Promise<RebootResult>; stopMining: (minerId: string) => Promise<RebootResult>; updateDevfee: (minerId: string, devfeePct: number | null) => Promise<DevfeeResult> }) {
  ...
  const doOpenRemote = async () => {
    if (!miner) return;
    setRemoteBusy(true); setRemoteFeedback(null);
    const error = await openRemoteTab(openRemote, miner.id);
    setRemoteBusy(false);
    setRemoteFeedback(error);
  };
    ...
    {miner ? <>
      ...
      {miner.licensed === false
        ...
        : <>
            ...
            {remoteReady && <div className="lm-detail-actions"><button className="lm-btn lm-hl-solid" disabled={remoteBusy} onClick={doOpenRemote}>{remoteBusy ? t("remoteOpening") : `🌐 ${t("remoteOpenMachine")}`}</button>{remoteFeedback && <span className="lm-err" role="alert">{remoteFeedback}</span>}</div>}
