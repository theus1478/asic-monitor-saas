...
export default async function FarmsPage() {
  ...
  return <Shell><PageHeader title={t("title")} description={t("description")} />
    <Link href="/collector" className="collector-banner"><span className="collector-icon">⇣</span><div><p className="eyebrow">{t("collectorBannerEyebrow")}</p><h2>{t("collectorBannerTitle")}</h2><p>{t("collectorBannerBody")}</p></div><b>{t("collectorBannerCta")}</b></Link>
    <section className="farm-list clean-list">
      {farmList.length === 0 && <article className="card empty-state"><span>▦</span><h2>{t("emptyTitle")}</h2><p>{t("emptyBody")}</p></article>}
      {farmList.map((farm) => {
