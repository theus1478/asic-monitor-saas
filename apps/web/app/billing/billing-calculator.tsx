...
import { averageUnitPriceCents, buildBscUsdtUri, monthlyPriceCents } from "../../lib/pricing";
...
export function BillingCalculator({ usedMachines, activeLicenses, pendingInvoice }: { usedMachines: number; activeLicenses: number; pendingInvoice: Invoice | null }) {
  ...
  const uri = useMemo(() => pendingInvoice ? buildBscUsdtUri(pendingInvoice.amount_usdt, pendingInvoice.wallet_address) : null, [pendingInvoice]);
  ...
  return <>
    ...
    {pendingInvoice && <section className="invoice-grid"><article className="card invoice-card"><p className="eyebrow">{t("chargeGeneratedEyebrow")}</p><h2>USDT {Math.abs(pendingInvoice.amount_usdt * 100 - Math.round(pendingInvoice.amount_usdt * 100)) < 1e-6 ? pendingInvoice.amount_usdt.toFixed(2) : pendingInvoice.amount_usdt.toFixed(6)}</h2><p>{t("sendExactAmount")}</p><span className="badge warning">{t("awaitingConfirmation")}</span><dl><div><dt>{t("chargeValidity")}</dt><dd>{new Date(pendingInvoice.due_at).toLocaleString(locale)}</dd></div><div><dt>{t("network")}</dt><dd>BNB Smart Chain (BEP-20)</dd></div><div><dt>{t("reference")}</dt><dd>{pendingInvoice.reference.slice(0, 18)}…</dd></div></dl><form action={verifyLicensePurchase.bind(null, pendingInvoice.id)}><button className="button" type="submit">{t("verifyPayment")}</button></form></article><article className="card pay-card"><p className="eyebrow">{t("payInUsdtEyebrow")}</p>{qr ? <Image src={qr} alt="QR code de pagamento USDT BEP-20" width={172} height={172} unoptimized className="qr-code" /> : <div className="qr-placeholder"><span>USDT</span></div>}<p>{t("afterConfirmation")}</p><code>{pendingInvoice.wallet_address}</code><button className="button secondary" type="button" onClick={copy}>{copied ? t("addressCopied") : t("copyAddress")}</button></article></section>}
  </>;
}
